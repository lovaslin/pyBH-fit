...
# Data
Nsig = int(0.0085 * Nbkg)
data = np.empty(Nbkg+Nsig)
if bkg_type == 'exp':
    data[:Nbkg] = np.random.exponential(3.5, Nbkg)
else:
    data[:Nbkg] = np.random.uniform(0, 35, Nbkg)
data[Nbkg:] = np.random.normal(6.5, 1.5, Nsig)
...
def fit_w(data, bkg, bins, w, lpath, tpath):
    ...
    if bh.t_ar[0]+2 > bbh[-1]:
        xcont = np.linspace(bbh[0], bh.t_ar[0]+2, 499)
    else:
        xcont = np.linspace(bbh[0], bbh[-1], 499)
    F = plt.figure(figsize=(12,8))
    plt.hist(bbh[:-1], bins=bbh, weights=Hbh, histtype='step', lw=2, label='pseudo-data (50k)')
    ...
    plt.xlabel('tdata', size='xx-large')
    plt.ylabel('Global p-value bias', size='xx-large')
    plt.xticks(fontsize='xx-large')
    plt.yticks(fontsize='xx-large')
    ...
    plt.xlabel('tdata', size='xx-large')
    plt.ylabel('Global significance bias', size='xx-large')
    plt.xticks(fontsize='xx-large')
    plt.yticks(fontsize='xx-large')
    ...
    plt.legend(fontsize='xx-large')
    plt.xlabel('tdata', size='xx-large')
    plt.ylabel('Global p-value', size='xx-large')
    plt.xticks(fontsize='xx-large')
    plt.yticks(fontsize='xx-large')
